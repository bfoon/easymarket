...
(function() {
    ...
    /**
     * Fetch the share URL and QR code from server
     */
    function fetchShareData() {
        ...
        fetch(shareUrlEndpoint)
            ...
            .catch(error => {
                console.warn('Share data fetch failed:', error);
            });
    }
    ...
    function attachEventListeners() {
        ...
        if (qrCodeBtn) {
            qrCodeBtn.addEventListener('click', handleShowQrCode);
        }
    }
    ...
    /**
     * Handle show QR code click
     */
    function handleShowQrCode() {
        if (!qrCodeData) {
            showToast('Generating QR code, please wait...', 'info');
            return;
        }
        ...
        if (qrCodeModal && typeof bootstrap !== 'undefined') {
